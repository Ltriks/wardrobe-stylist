// Stable coarse IDs for local recognition and standalone board layout.
// Editable family categories live in ClothingCategory; see category-catalog.ts.
...
export function categoryBoardGroup(category: string): BoardGroup {
  if (category === 'whole') return 'whole';
...
export function suggestClothingCategory(name: string): Category {
  ...
  if (/家居|睡衣|睡裤|睡袍|连体衣|爬服|哈衣|\b(pajamas?|pyjamas?|loungewear|sleepwear|nightgown|nightdress|bathrobe|romper|onesie|bodysuit)\b/.test(text)) return 'loungewear';
  ...
  if (/裤|\b(pants|jeans|trousers|bottom|shorts|leggings|slacks|denim|sweatpants)\b/.test(text)) return 'bottom';
  if (/上衣|衬衫|短袖|长袖|卫衣|毛衣|背心|\b(tshirt|t shirt|shirt|top|blouse|tank|tee|sweater|hoodie|pullover|jersey|sweatshirt)\b/.test(text)) return 'top';
